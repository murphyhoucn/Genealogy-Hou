...
// 解析生平事迹内容
function parseBiographyContent(bio: string): string {
    if (!bio) return "";
    ...
    try {
        ...
        if (textContent.length > 0) {
            ...
            return JSON.stringify(richTextFormat);
        }
    }
...
const MemberPage = memo(function MemberPage({
    ...
    pageIndex: number;
    totalPages: number;
    formatDate: (year: number | null) => string;
}) {
    return (
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-stone-50/50 rounded-md p-3 sm:p-4 border border-stone-100">
                        <div className="[&_*]:!text-stone-700">
                            <RichTextViewer
                                key={member.id}
                                            value={parseBiographyContent(member.bio)}
                                animate={false}
...
export function BiographyBook({ members }: BiographyBookProps) {
    ...
    const formatDate = useCallback((year: number | null) => {
        if (!year) return "-";
        return `${year}年`;
    }, []);
